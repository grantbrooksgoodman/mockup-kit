...
///
/// Grouped positions like ``above(spacing:)`` and ``below(spacing:)`` move with the
/// primary device frame's ``DeviceFrame/offset``, while canvas-anchored positions like
/// ``canvasTop(padding:)`` and ``canvasBottom(padding:)`` remain fixed relative to the
/// canvas edges.
public enum HeadlinePosition: Sendable {
    /// Positioned above the primary device frame with the specified spacing.
    ///
    /// The headline and primary device frame are laid out as a vertical group,
    /// with the headline on top. The frame's ``DeviceFrame/offset`` applies to the
    /// entire group.
    case above(spacing: CGFloat = 40)

    /// Positioned below the primary device frame with the specified spacing.
    ///
    /// The headline and primary device frame are laid out as a vertical group,
    /// with the headline on the bottom. The frame's ``DeviceFrame/offset`` applies
    /// to the entire group.
    ...
    /// Vertically centered between the bottom edge of the primary device frame
    /// and the bottom edge of the canvas.
    ///
